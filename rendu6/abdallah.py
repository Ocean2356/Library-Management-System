...
def affichage_sanction(cur):
    ...
    while user_choix != "0":
        ...
        print("Le montant du remboursement est de : ", raw[i_sanction][5], "€")
...
def ajouter_sanction(cur):
    # login
    print("Entrer le login de l'adhérent à sanctionner")
    ...
    while (remboursement_du != 0) and (remboursement_du != 1):
        ...
        remboursement_du = int(input("Saisir un choix : "))
    # SQL
    sql = "insert into sanction(adherent,exemplaire,code_ressource,date_pret,duree_sanction,remboursement,remboursement_du ) values ('%s','%s','%s','%s','%s','%s','%s');" % (login_adh, exemplaire, code_ressource, date(Nyear_pret, Nmonth_pret, Nday_pret), duree_sanction, remboursement, remboursement_du)
    cur.execute(sql)
...
def ajouter_adherent(cur):
    ...
    # Datetime
    Nyear = int(input("Veuillez saisir l'année de naissance de l'adhérant : "))
    Nmonth = int(input("Veuillez saisir le mois de naissance de l'adhérant : "))
    Nday = int(input("Veuillez saisir le jour de naissance de l'adhérant : "))
    ...
    if id_adherent != "" and nom != "" and prenom != "" and adresse != "" and email != "" and login != "" and mot_de_passe != "" and tel != "":
        ...
        if raw:
            ...
            # Ajout de l'adhérent
            sql = "INSERT INTO adherent (numero_carte, nom, prenom, date_de_naissance, adresse, email, tel, actuelle) VALUES ('%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % (id_adherent, nom, prenom, date(Nyear, Nmonth, Nday), adresse, email, tel, True)
            cur.execute(sql)
            # Creation compte Nouvel Adhérent non blacklisté par défaut
            sql = "INSERT INTO compte_adherent (login, mot_de_passe, blackliste, adherent) VALUES ('%s', '%s', '%s', '%s');" % (login, mot_de_passe, False, id_adherent)
            cur.execute(sql)
